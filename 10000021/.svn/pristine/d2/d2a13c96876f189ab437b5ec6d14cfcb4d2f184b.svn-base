...
var PkApi = (function () {
     ...
     * @p-config    {Function}  prepare(dataToShare) 准备数据，在这里可以自定义要分享的数据。dataToShare 对象里包含一个名为content 的字符串和一个名为url 的URL，供自定义。
    ...
    function pkRegisterShare(callbacks) {
        ...
        var _callShare = function (dataToShare) {
            PKJSBridge.invoke('pkShare', dataToShare, function (resp) {
                switch (resp.err_msg) {
        ...
        PKJSBridge.on('menu:share', function (dataToShare) {
    ...
    function pkAddFavorite() {
        PKJSBridge.call('startPKAddFavorite');
    }
    ...
    function pkSaveGameStatus(gameStatus) {
        PKJSBridge.call('saveGameStatus', gameStatus);
    }
    ...
    function pkLoadGameStatus(callback) {
        PKJSBridge.invoke('loadGameStatus', '', callback);
    }
