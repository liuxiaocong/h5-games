...
lg.startGame = function(scene){
    ...
    lg.preload(g_resources, function(){
        var splashing = false;
        if(a10Enabled) {
            if(a10Remote) {
...
//    if(lg.currentSceneName == sceneName) return;
    ...
    lg.preload(s.res,function(){
        ...
        if(logoButton) {
            lg.inputManager.removeListener(logoButton);
            logoButton.removeFromParent(false);
            lg.currentScene.addChild(logoButton, 999999);
            logoButton.setPosition(0, 0);
            if(lg.currentSceneName == "levelSelect") logoButton.setPositionX(lg.stage.center().x - 80);
            lg.inputManager.addListener(logoButton, goMoreGame);
        }
    });
}
//----------------------scene about-------------------------------------------------------
