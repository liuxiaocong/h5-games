...
(function() {
    ...
    var p = LevelOne.prototype = new createjs.Container();
    ...
    p.initLevel = function(){
        ...
        if(!this.back){
            this.back = new createjs.Bitmap( queue.getResult("level_1_background") );
        }
        this.addChild(this.back);

        // площадки
        if(!this.landing_red){
            this.landing_red = new Landing( queue.getResult("level_1_1"), 144, 148, 259, 13, "red" );
        }
        this.addChild(this.landing_red);
        if(!this.landing_red_area){
            this.landing_red.landingWaypoints = [new point(170, 155), new point(185, 155), new point(200, 155), new point(215, 155)];
            this.landing_red_area = new createjs.Shape();
            this.landing_red_area.x = 140;
            this.landing_red_area.y = 145;
            this.landing_red_area.graphics.beginFill("#00FF00").drawRect(0,0, 24,19);
            this.landing_red_area.alpha = 0.0;
        }
        this.addChild(this.landing_red_area);

        if(!this.landing_yellow){
            this.landing_yellow = new Landing( queue.getResult("level_1_2"), 305,136, 91, 91, "yellow" );
        }
        this.addChild(this.landing_yellow);
        if(!this.landing_yellow_area){
            this.landing_yellow.landingWaypoints = [new point(328, 205), new point(338, 195), new point(348, 185), new point(358, 175)];
            this.landing_yellow_area = new createjs.Shape();
            this.landing_yellow_area.x = 318;
            this.landing_yellow_area.y = 202;
            this.landing_yellow_area.graphics.beginFill("#00FF00").drawRect(0,0, 18,25);
            this.landing_yellow_area.alpha = 0.0;
            this.landing_yellow_area.rotation = 45;
        }
        this.addChild(this.landing_yellow_area);

        if(!this.landing_blue){
            this.landing_blue = new Landing( queue.getResult("level_1_3"), 272,108,31,31, "blue" );
        }
        this.addChild(this.landing_blue);
        if(!this.landing_blue_area){
            this.landing_blue.landingWaypoints = [new point(287, 123)];
            this.landing_blue_area = new createjs.Shape();
            this.landing_blue_area.x = 272;
            this.landing_blue_area.y = 108;
        ...
        //this.labelScore = new createjs.Text(this.landedText+": 0", "15px CooperBlackBold", "rgba(255, 255, 255, 1)");
    ...
    p.showTutorial = function(){
        ...
        //this.TutorialText = new createjs.Text(text, "17px CooperBlackBold", "rgba(69, 69, 70, 1)");
        ...
        var img = ok_image;
        this.TutorialButtonOk = new createjs.Bitmap(queue.getResult(img));
        var butY = 230;
        var butX = 360;
        if(iPhone){
            butY = 210;
    ...
    p.setCollider = function(){
        ...
        this.colliderTimer = setInterval(function()
        {
            ...
            if(needToGameOver){
                ...
                clearInterval(self.difficultTimer);
    ...
    p.onTick = function(event) {
        ...
        for(var activePlaneIndex = 0; activePlaneIndex<this.planes.length; activePlaneIndex++)
        {
            ...
            if(activePlane.active && activePlane.needForDelete){
                ...
                this.removeChild(activePlane.waypointsContainer);
