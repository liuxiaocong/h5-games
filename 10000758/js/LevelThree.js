...
(function() {
    ...
    var p = LevelThree.prototype = new createjs.Container();
    ...
    p.initLevel = function(){
        ...
        if(!this.back){
            this.back = new createjs.Bitmap( queue.getResult("level_3_background") );
        }
        this.addChild(this.back);

        // площадки
        if(!this.landing_red){
            this.landing_red = new Landing( queue.getResult("level_3_1"), 138, 70, 156, 73, "red" );
        }
        this.addChild(this.landing_red);
        if(!this.landing_red_area){
            this.landing_red.landingWaypoints = [new point(168, 126), new point(179, 121), new point(190, 117), new point(201, 112)];
            this.landing_red_area = new createjs.Shape();
            this.landing_red_area.x = 157;
            this.landing_red_area.y = 122;
            this.landing_red_area.graphics.beginFill("#00FF00").drawRect(0,0, 15,22);
            this.landing_red_area.alpha = 0.0;
            this.landing_red_area.rotation = 68;
        }
        this.addChild(this.landing_red_area);

        if(!this.landing_yellow){
            this.landing_yellow = new Landing( queue.getResult("level_3_2"), 196, 115, 107, 87, "yellow" );
        }
        this.addChild(this.landing_yellow);
        if(!this.landing_yellow_area){
            this.landing_yellow.landingWaypoints = [new point(275, 139), new point(265, 147), new point(255, 155), new point(245, 163)];
            this.landing_yellow_area = new createjs.Shape();
            this.landing_yellow_area.x = 294;
            this.landing_yellow_area.y = 115;
            this.landing_yellow_area.graphics.beginFill("#00FF00").drawRect(0,0, 13,19);
            this.landing_yellow_area.alpha = 0.0;
            this.landing_yellow_area.rotation = 48;
        }
        this.addChild(this.landing_yellow_area);

        if(!this.landing_blue){
            this.landing_blue = new Landing( queue.getResult("level_1_3"), 281,163,31,31, "blue" );
        }
        this.addChild(this.landing_blue);
        if(!this.landing_blue_area){
            this.landing_blue.landingWaypoints = [new point(293, 184)];
            this.landing_blue_area = new createjs.Shape();
            this.landing_blue_area.x = 281;
            this.landing_blue_area.y = 163;
        ...
        this.labelScore = new createjs.Text(this.landedText+": 0", "15px Helvetica, Arial, Verdana, Microsoft Yahei, 微软雅黑, STXihei, 华文细黑, sans-serif", "rgba(255, 255, 255, 1)");        this.labelScore.x = 10;
